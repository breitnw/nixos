# patched rclone module to support `rclone sync`
# delete this if it is ever supported in home-manager
{
  config,
  lib,
  pkgs,
  ...
  cfg = config.programs.rclone-custom;
      ...
      (
        ...
        in
          lib.concatMap (
            ...
            in [
              # timer job
              (lib.nameValuePair "rclone-sync:${replaceSlashes dst-path}@${remote-name}" {
                Unit = {
                  Description = "Rclone sync timer for ${remote-name}:${dst-path}";
                };
                Timer = {
          ...
          in
            lib.concatMap (
              ...
              in [
                ...
                (lib.nameValuePair "rclone-sync:${replaceSlashes dst-path}@${remote-name}" {
                  Unit = {
                    Description = "Rclone sync job for ${remote-name}:${dst-path}";
                  };
