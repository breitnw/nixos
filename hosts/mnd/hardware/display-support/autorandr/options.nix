# patched autorandr to enable filtering

{ config, lib, pkgs, ... }:

let

  cfg = config.services.autorandr-custom;
    ...
    options = {
      ...
      rotate = lib.mkOption {
        type = lib.types.nullOr
          (lib.types.enum [ "normal" "left" "right" "inverted" ]);
      ...
      filter = lib.mkOption {
        type = lib.types.nullOr (lib.types.enum [ "bilinear" "nearest" ]);
        description = "Output filter configuration.";
        default = null;
        example = "bilinear";
      };
  ...
  configToString = name: config:
    if config.enable then
      lib.concatStringsSep "\n" ([ "output ${name}" ]
        ...
        ++ lib.optional (config.filter != null) "filter ${config.filter}"
...
in {
  ...
  options = {
    ...
    services.autorandr-custom = {
      enable =
        lib.mkEnableOption "handling of hotplug and sleep events by autorandr";
